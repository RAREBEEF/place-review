# Place Review
![](https://imagedelivery.net/v7-TZByhOiJbNM9RaUdzSA/2578390b-1c0e-460a-a9f9-05ba11e17f00/public)
...
사이트 이름은 Place Review로 지었는데, 개인 프로젝트를 진행하는데 있어서 사이트 이름 짓는데 큰 시간을 할애하지는 않기 때문에 그냥 직관적으로 작명하였다.
...
막상 모아놓고 보니... 썩 그렇게 보기 좋은 모습으로 모으진 못했지만 멀쩡하게 작동하는 모습을 보니 뿌듯하긴 했다.
...
- 전체 리뷰 / 마커 위치 리뷰 **두 가지의 필터**를 통해 리뷰를 확인할 수 있다.
- 주소나 제목, 내용 등을 통해 **리뷰를 검색**할 수 있다.
- **리뷰를 클릭**하면 지도가 해당 **리뷰 위치로 이동**한다.
...
- 리뷰의 제목은 **선택한 장소 검색 결과의 가게 이름으로 자동 변경**된다. 필요에 따라서 직접 입력할 수도 있다. 최대 12자까지 작성 가능하다.
...
- 프로필 페이지에서 내가 작성한 리뷰들을 확인할 수 있도록 하여 관리가 용이하다.
...
또한 툴바의 모든 기능과 지도를 최대한 한 화면에 담을 수 있도록 검색 결과는 웹 전체가 아닌 각각의 영역 스크롤을 통해 둘러볼 수 있도록 만들었다.
...
## 1. React, TypeScript
React 기반, TypeScript로 구현한 프로젝트이다. 
...
```
지도를 삽입할 요소와 옵션을 인자로 전달한다. 옵션으로는 지도 시작 위치와 확대 비율 등을 전달할 수 있다.
...
콜백함수의 인자는 검색 결과, 상태, 페이지네이션으로 이루어진다. 상태에 따라 검색 결과를 처리하고 검색 결과가 많을 경우 페이지네이션을 통해 페이지를 설정할 수 있다.

추가로 다양한 옵션을 지정할 수 있는데, 그 중 `location` 옵션에 현위치 정보를 전달하여 위치 정보 액세스 허용일 경우 현위치 근처 우선으로 검색 결과를 출력하도록 하였다.

결과로 전달되는 데이터는 배열이며, 배열 내부에 있는 각각의 객체각 담고있는 정보는 아래와 같다.
...
현위치 좌표 및 지도 중심 위치(마커 위치)의 좌표를 주소로 변환하여 검색하는 기능을 구현하는데 사용하였다.
...
물론 투자한 시간에 비해 깔끔하게 다듬어지지는 않았지만 코드를 다듬는데 많은 시간을 투자하는 것은 큰 공부가 된다고 생각하고 또 실제로 직접 작성한 코드를 여러차례 곱씹어 볼 수 있는 시간이었기 때문에 알차게 보냈다고 생각한다.
